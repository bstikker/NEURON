...
if (length(args) < 3) {
  stop("Usage: Rscript aggregate_sturgeon_v2_timecourse_fixed.R <sample_dir> <output_pdf> <summary_tsv> [read_counts_tsv]")
}
...
# One fixed palette across both plots so shared classes keep the same color.
palette_keys <- sort(unique(all_df$display_class))
fixed_palette <- setNames(grDevices::hcl.colors(length(palette_keys), palette = "Dark 3"), palette_keys)
...
plot_classifier <- function(df, title_text, ylab_text) {
  ...
  ggplot(plot_df, aes(x = time_min, y = confidence, color = display_class, group = display_class)) +
...
if (nrow(cnv_df) > 0) {
  for (i in seq_len(nrow(cnv_df))) {
    ...
    if (!is.null(img)) {
      ...
      # Slightly narrower and taller drawing box so the CNV panels no longer look
      # stretched in width compared with the final diagnostic report.
      page_width_npc  <- 0.84
      page_height_npc <- 0.68
